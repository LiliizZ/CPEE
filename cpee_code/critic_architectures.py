...
class joint_critic(object):
    ...
    def conv(self, x, z):
        ...
        hidden_dim = 128
        hidden_dim1 = 256
        #keep_prob = tf.placeholder(tf.float32, name='gru_keep_prob')
        ...
        with tf.variable_scope('critic', reuse=tf.AUTO_REUSE):
            ...
            print("+++++++++++++h1", h1.shape)#?,200,300,32
            print("++++++++++++=h2", h2.shape)#?,200,300,64
            ...
            def dropout(hidden_dim):
                cell = gru_cell(hidden_dim)
                return tf.contrib.rnn.DropoutWrapper(cell)

            '''cells = [dropout(hidden_dim) for _ in range(5)]
            cells1 = [dropout(hidden_dim1) for _ in range(5)]
            rnn_cell = tf.contrib.rnn.MultiRNNCell(cells, state_is_tuple=True)
            rnn_cell1 = tf.contrib.rnn.MultiRNNCell(cells1, state_is_tuple=True)
            a0, _ = tf.nn.dynamic_rnn(cell=rnn_cell, inputs=a, dtype=tf.float32)
            b0, _ = tf.nn.dynamic_rnn(cell=rnn_cell1, inputs=b, dtype=tf.float32)
            print("a0", a0.shape)
            print("b0", b0.shape)'''
            a0 = a
            b0 = b
            #h0 = self.critic_activation(tf.add(a0, b0))
            h0 = self.critic_activation(tf.concat([a0, b0], axis=1))
            print("h0", h0.shape)

            h1 = tf.nn.relu(h0)
            h1 = tf.reduce_mean(h1, axis=1)
            for i, unit in enumerate(self.critic_layers):
                h = tf.layers.dense(h1, unit, activation=self.critic_activation, name="fc_{}".format(i),
                                    kernel_initializer=init_weigths)  # , bias_initializer=init_bias)
