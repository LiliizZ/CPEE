        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse=True):
            ...
            with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
                if optimizer is None:
                    optimizer_mi1 = tf.train.AdamOptimizer(learning_rate=self.critic_lr, name="optim_mi1")
                    train_ops['critic'] = optimizer_mi1.minimize(- mi_for_grads, var_list=self.vars['critic'], name="mi1_mini")
